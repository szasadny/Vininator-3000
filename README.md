...
A machine learning project that combines the [X-Wines](https://github.com/rogerioxavier/X-Wines) dataset with [ERA5-Land](https://www.ecmwf.int/en/era5-land) growing-season weather data (pulled via [Open-Meteo](https://open-meteo.com/)) and [SoilGrids](https://soilgrids.org/) soil and terrain data to predict:
...
Weather data is pulled from the [Open-Meteo Historical Weather API](https://open-meteo.com/en/docs/historical-weather-api), which needs no account or API key for the free tier. (The free tier is non-commercial; Vininator is a personal/learning project, so this is fine.)
...
# 2. Pull Open-Meteo weather (cached, resumable; first run ~12min for the full variant)
...
src/vininator/
  ...
  features/     Climate (Open-Meteo), soil & terrain (SoilGrids + DEM), terroir joiner, text parsing, feature assembly
...
**Weather (Phase 2):** ERA5-Land daily reanalysis (0.1° / ~11 km) pulled per region via the [Open-Meteo Historical Weather API](https://open-meteo.com/en/docs/historical-weather-api). Free, no registration. Open-Meteo CC BY 4.0; non-commercial use only on the free tier.
...
- **Weather data** — [Open-Meteo](https://open-meteo.com/) (CC BY 4.0), serving [ERA5-Land](https://www.ecmwf.int/en/era5-land) reanalysis from the Copernicus Climate Change Service (C3S).
